...
class OpenAIController extends Controller
{
    ...
    public function text_to_speech(Request $request)
    {
        ...
        try {
            $app = AppSetting::first();
            if (strlen($app->aws_key) <= 0 || strlen($app->aws_secret) <= 0) {
                return back()->with('error', 'AWS Key and Secret are not set yet.');
            }
            $outputFormat = 'mp3';
            $todaysSpeeches = AppHelper::today_content('speech');
            $contentOver = AppHelper::limit_checker($todaysSpeeches, 'text_to_speech_generation', 'text to speech');
            if ($contentOver) {
                return back()->with('error', $contentOver['error']);
            };

            if ($this->user()->role == "user") {
                $textLength = strlen($request->description);
                $totalLength = $this->user()->subscription_plan->text_character_length;

                if ($totalLength != "Unlimited" && (int) $totalLength < $textLength) {
                    return back()->with('error', "Max tex character limit is $totalLength characters.");
                }
            }
            ...
            $result = $client->synthesizeSpeech([
                ...
                'VoiceId' => $request->voice, // Specify the voice to use
                'LanguageCode' => $request->language,
            ]);

            AppHelper::content_handler($todaysSpeeches, 'speech');

            // Save the audio file locally
            $audioPath = 'text-to-audio/' . time() . '.' . $outputFormat;
            file_put_contents(public_path($audioPath), $result['AudioStream']->getContents());
